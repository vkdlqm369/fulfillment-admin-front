      ...
      <v-row justify="center" class="fill-height" >
        <v-col cols="12" md="8" lg="6" class="d-flex flex-column fill-height overflow-auto">
          ...
          class="flex-grow-1 overflow-y-auto" style="height: 100vh;">
            ...
            <v-dialog v-model="dialog4" max-width="450" max-height="300"persistent>
                  <v-card class="pa-2">
                    <v-card-title class="justify-center">
                      등록이 완료되었습니다<br>관리자 조회 페이지로 이동합니다.
                    </v-card-title>
                  <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn @click="dialog = true" to="/search">확인</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
...
const email = ref('');
const permission = ref('MASTER');
const name = ref('sss');
const department = ref('');
const memo = ref('');
const show1 = ref(false);
const show2 = ref(false);
const dialog = ref(false); //'목록으로' 모달
const dialog2 = ref(false);//'채우지 않은 값' 모달
const dialog3 = ref(false);//'유효하지 않은 값' 모달
const dialog4 = ref(false);//'등록 완료' 모달
const items = [
...
const submitForm = () => {
  if (!enteredForm()) {
    ...
    handleSubmit();// 폼 제출 처리 로직 (예: 서버에 데이터 전송)
